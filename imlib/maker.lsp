...
(setq targets '(("image"                                    ; target name
		 ("filter"   "gifread" "globals" "image"  "linked" "input"
		  ...
		  "jrand"    "lbmread" "targa"   "keys"    "dprint" "status"  "visobj")
		 self
		...
		("sound"    ("port/unix/sound")          self ("include") "" (SUN))
		("sound"    ("port/aix/sound" "readwav") self ("include") "" (AIX))
		("sound"    ("port/sgi/sound" "readwav") self  ("include") "" (SGI))
		("joy"    ("port/unix/joystick")         self ("include") "" (AIX SUN SGI))
		...
		("joy"    ("port/linux/joystick") self ("include") "" (LINUX_X11 LINUX_SVGA))
