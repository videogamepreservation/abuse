...
short OldEventMask;
EventRecord MacEv;
int mmoved,mbed,mac_ready,keyed,mouse2ed;
...
int event_handler::event_waiting()
{
	...
	if (ewaiting || mmoved || mbed || mac_ready || keyed)
		return 1;
	...
	mouse->update();
	mouse->set_button(mouse->button() | (mouse2ed << 1));
	if (mouse->lastx() != mouse->x() || mouse->lasty() != mouse->y())
	{
		mmoved = 1;
	}
	if (mouse->last_button() != mouse->button())
	{
		mbed = 1;
	}
	...
	return ewaiting || mmoved || mbed || mac_ready || keyed;
}
        ...
        switch ((MacEv.message & keyCodeMask)>>8)
        {
          ...
          */
          ...
          case 0x6d :   ev.key=JK_F10; break;
      		...
      		case 0x52: // generate mouse 2 event
      			mouse2ed = (ev.type == EV_KEY);
      			ev.type = EV_MOUSE_BUTTON;
 						mouse->set_button((mouse->button()&1) | (mouse2ed << 1));
     				ev.mouse_button |= mouse->button();
     				mbed = 1;
      			break;
          default :
