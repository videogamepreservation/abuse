...
void view::get_input()
{
  ...
  if (the_game->key_down(lnumber_value(symbol_value(l_left_key))))
    sug_x=-1;
  else if (the_game->key_down(lnumber_value(symbol_value(l_right_key))))
    sug_x=1;
  else sug_x=0;

  if (the_game->key_down(lnumber_value(symbol_value(l_up_key))))
    sug_y=-1;
  else if (the_game->key_down(lnumber_value(symbol_value(l_down_key))))
    sug_y=1;
  else sug_y=0;

  if ((last_demo_mbut&2) || the_game->key_down(lnumber_value(symbol_value(l_special_key))))
    sug_b1=1;
  else sug_b1=0;
...
int view::handle_event(event &ev)
{
  if (ev.type==EV_KEY)
  {
		if (ev.key == lnumber_value(symbol_value(l_weapon_left_key)))
    {
    	if (total_weapons) 
    		last_weapon(); 
    	return 1; 
    }
 		else if (ev.key == lnumber_value(symbol_value(l_weapon_right_key)))
    { 
    	if (total_weapons) 
    		next_weapon(); 
    	return 1;
